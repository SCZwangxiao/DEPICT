import sys
sys.path.append('/home/wangxiao24/dev_videochat/tools/data/bilibili/LLaMA-VID')
sys.path.append('/home/wangxiao24/dev_videochat/tools/data/bilibili/LLaVA')
import os
import json
import time
...
class VideoLLMProcessor:
    ...
    def __init__(self, 
        ...
        self.image_processor, self.tokenizer = self.get_tokenizer_processor(model_path, mm_use_im_start_end)
    ...
    @staticmethod
    def get_tokenizer_processor(model_path, mm_use_im_start_end):
    ...
    def __call__(self, anno):
        ...
        try:
            ...
            for ablation_type in self.ablation_types:
                ...
                # Load video
                video = self.load_video(video_path, self.num_frames)
...
class VideoLLMWorker(mp.Process):
    ...
    @staticmethod
    def build_model(device, model_path, model_base):
        print('Init model in %s' % device)
        model_name = get_model_name_from_path(model_path)
        tokenizer, model, _, _ = load_pretrained_model(model_path, model_base, model_name, device_map=device)
    ...
    def run(self):
        tokenizer, model = self.build_model(self.device, self.model_path, self.model_base)
...
def update_config(model_path, clip_path, clip_processor_path):
    old_config_path = os.path.join(model_path, 'config.json')
    new_config_path = os.path.join(model_path, 'config_new.json')
    configs = json.load(open(old_config_path, 'r'))
    configs['mm_vision_tower'] = clip_path
    configs['image_processor'] = clip_processor_path
    json.dump(configs, open(new_config_path, 'w'))
    os.system(f'mv {new_config_path} {old_config_path}') # Safe write
...
if __name__ == "__main__":
    ...
    model_base = None
    update_config(model_path, clip_path, clip_processor_path)
    ...
    for gpu_id in range(num_gpus):
        ...
        proc = VideoLLMWorker(task_queue, result_queue, device, model_path, model_base)
        proc.start()
        procs.append(proc)
